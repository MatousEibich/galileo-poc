...
from config import APP_ICON, APP_TITLE, LOGO_PATH
        ...
        with st.sidebar:
            ...
            if "history" in st.session_state and st.session_state.history:
                try:
                    ...
                    st.download_button(
                        "⬇️ Stáhnout přepis", transcript, "chat_přepis.txt", "text/plain"
                    )
...
def initialize_session_state() -> None:
    ...
    try:
        if "history" not in st.session_state:
            ...
            logger.debug(
                f"Chat history already exists with {len(st.session_state.history)} messages"
            )
