...
def grade_responses(responses: list, questions_dict: dict) -> list:
    ...
    """
    print(f"Grading {len(responses)} responses using LLM-as-a-judge with ground truth comparison...")
    ...
    for i, response in enumerate(responses):
        print(f"[{i+1}/{len(responses)}] Grading question {response['id']}: {response['question'][:50]}...")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Grade agent responses using LLM-as-a-judge with ground truth")
    ...
    parser.add_argument("--questions", "-q", default="eval_questions_with_ground_truth.json", 
                       help="JSON file with questions and ground truth (default: eval_questions_with_ground_truth.json)")
    ...
    if args.output is None:
        ...
        output_file = Path(__file__).parent / "results" / "grades" / f"eval_grades_with_ground_truth_{timestamp}.json"
    else:
