...
# Load the JSON file
with open('official_boards_tyn.json', 'r', encoding='utf-8') as file:
    data = json.load(file)
...
# Save to CSV
csv_path = 'official_boards_tyn.csv'
df.to_csv(csv_path, index=False, encoding='utf-8')
print(f'Saved CSV to {csv_path}')

# Save to Excel (XLSX)
excel_path = 'official_boards_tyn.xlsx'
...
df_excel_friendly['content'] = df_excel_friendly['content'].apply(lambda x: (x[:500] + '...') if isinstance(x, str) and len(x) > 500 else x)
excel_friendly_path = 'official_boards_tyn_excel_friendly.xlsx'
